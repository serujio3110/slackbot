...
class Driver(object):
    ...
    def _send_message_to_bot(self, channel, msg):
        self.clear_events()
        self._start_ts = time.time()
        self.webapi.chat_postMessage(channel=channel, text=msg, username=self.driver_username, unfurl_links=False, unfurl_media=False)
    ...
    def _has_got_message(self, channel, match, start=None, end=None):
        ...
        response = self.webapi.conversations_history(channel=channel, oldest=oldest, latest=latest)
    ...
    def _join_test_channel(self):
        response = self.webapi.conversations_join(channel=self.test_channel)
        self.cm_chan = response['channel']['id']
        self._invite_testbot_to_channel()

        private_channels = self.webapi.conversations_list(channel=self.test_private_channel, types="private_channel")["channels"]
        for private_channel in private_channels:
            if self.test_private_channel == private_channel['name']:
                self.gm_chan = private_channel['id']
                self._invite_testbot_to_private_channel()
                break
        else:
    ...
    def _invite_testbot_to_channel(self):
        if self.testbot_userid not in self.webapi.conversations_members(channel=self.cm_chan)['members']:
            self.webapi.conversations_invite(channel=self.cm_chan, users=self.testbot_userid)

    def _invite_testbot_to_private_channel(self):
        if self.testbot_userid not in self.webapi.conversations_members(channel=self.gm_chan)['members']:
            self.webapi.conversations_invite(channel=self.gm_chan, users=self.testbot_userid)
